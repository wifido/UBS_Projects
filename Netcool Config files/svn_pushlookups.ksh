...
# Mahesh Kumar 11Mar2009 - Modified to use lookupdate from lookupset
#######################################################################################################
...
# Arguments passed from svn_lookup_push.ksh
server=$1
rel=$2
lookup=$3
enviro=$4
...
if [ "$lookuplength" -gt 25 ]; then
    lookupdate=`echo $lookup|cut -c13-22`
else
    lookupdate=`echo $lookup|cut -c13-20`
fi

echo "release date - $reldate"
echo "lookup date - $lookupdate"
...
if [ "$is_ism" != "" ]
	...
	echo "Updating $rel\n"
	cd $rel
	svn update $lookup --username="sso_esmmon" --password="orange18"
	pb-chown -R netcool:NCO_Superuser $lookup
        # Check if lookuptables symbolic link exists and act accordingly.
        if [ -L lookuptables ]; then
            echo "lookuptables is a sym link"
            #Get the current lookuptables version
            link=`ls -lh lookuptables | awk '{print $11}'`
            if [ "$link" = $lookup ]; then
                # We are already using target lookup release. Do nothing.
                echo "lookuptables already sym links to $lookup. Please check"
            else
                # We are using a different version. Point symlink to target lookuptables.
                echo "lookuptables points to $link. Changing sym link"
                pb-rm -rf lookuptables
                pb-ln -s $lookup lookuptables
                pb-chown -R netcool:NCO_Superuser lookuptables
            fi
        else
            # lookuptables is not a symlink. Creating symlink lookuptables forcefully.
            echo "lookuptables file either does not exist or is not a sym link. Please check."
            pb-rm -rf lookuptables
            pb-ln -s $lookup lookuptables 
        fi
        cd ../
...
do
	...
	version=`grep probe_lookup_version $Logfile|tail -1|awk '{print $NF}'`
		if [ -f $Logfile ]
		then
			...
			version=`grep probe_lookup_version $Logfile|tail -1|awk '{print $NF}'`
			   ...
			   then
			      ...
			      then
			         ...
			         version=`grep probe_lookup_version $OldLogfile|tail -1|awk '{print $NF}'`
			      fi
				if [ "$version" == "" ]
				then 
			   ...
			   sleep 5
			   newversion=`grep probe_lookup_version $Logfile|tail -1|awk '{print $NF}'`
				if [ "$newversion" != "" ]
				then
					if [ "$newversion" != "$version" ] && [ "$newversion" == "$lookupdate" ]
					then

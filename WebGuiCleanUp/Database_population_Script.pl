...
$DIRNAME='/sbclocal/tivoli/webgui/etc/maps';
@MAPFILES=`ls -1 $DIRNAME/*.map`;
...
$SFxml = new XML::Simple (KeyAttr=>[],Variables=>{name => value},ForceArray =>1);
$GFxml = new XML::Simple (KeyAttr=>[],Variables=>{name => value},ForceArray =>1);
#$xml = new XML::Simple (ForceArray =>['dependency']);
$SFdata = $SFxml->XMLin("/sbclocal/tivoli/webgui/etc/data/system/filter.xml");
$GFdata = $GFxml->XMLin("/sbclocal/tivoli/webgui/etc/data/global/filter.xml");
...
#
#########################################################################
open (INFILE ,"/sbclocal/tivoli/webgui/etc/data/system/view.xml");

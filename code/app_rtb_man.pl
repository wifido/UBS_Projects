...
sub DevVars
{
	...
	our $LIB_AppLogFileName="CMDB_Update_test";
...
#	                        "reporter",
	...
	my $sthOracle = $dbh->prepare("SELECT CMDB_APP_ID, RTB_APPLICATION_MANAGER, Class FROM managed_app where CMDB_APP_ID is not NULL") || die $dbh->errstr;
	...
	while (my (@nodeOracle) = $sthOracle->fetchrow_array) 
	{
			...
			$LenRtbAppMan = length($RtbAppMan);
			$CLASS_ID = $nodeOracle[2];
			if (exists($CMDB_Hash{$CMDB_ID}))
			{
				$CMDB_APP_rtb_man = $CMDB_Hash{$CMDB_ID};
				$CMDB_APP_rtb_man =~ s/'/ /;
				if($RtbAppMan ne $CMDB_APP_rtb_man)
				{
					if ($Verbose){print "CMDB_ID = $CMDB_ID   RtbAppMan = \'$RtbAppMan\' CMDB_APP_rtb_man = $CMDB_APP_rtb_man   $LenRtbAppMan\n";}
					logit ("RTB_APPLICATION_MANAGER changed for Class $CLASS_ID from $RtbAppMan to $CMDB_APP_rtb_man \n");
					if($Testing != 1)
					{
						my $sthOracle = $dbh->prepare("update managed_app set RTB_APPLICATION_MANAGER = '$CMDB_APP_rtb_man', CMDB_UPDATE = sysdate where CMDB_APP_ID = $CMDB_ID") || die $dbh->errstr;
						$sthOracle->execute() || die $sthOracle->errstr;
					}
				}
			}
...
#	$dbh = DBI->connect( "dbi:Oracle:database=DCMDBV2;HOST=sldn1339dor.ldn.swissbank.com;PORT=1524;SID=DCMDBV2",
	...
	my $sthOracle = $dbh->prepare("SELECT Application_id, UBS_RTBCONTACTNAME FROM aradmin.ubs_esm_cmdb_appsys ") || die $dbh->errstr;
	...
	while (my (@nodeOracle) = $sthOracle->fetchrow_array) 
	{
		...
		{
			$CMDB_ID = $nodeOracle[0];
			$RTBCONTACTNAME = $nodeOracle[1];
			$CMDB_Hash{$CMDB_ID} = $RTBCONTACTNAME;
		}
	}
	$sthOracle->finish;
	$dbh->disconnect();

...
#	                        "reporter",
	...
	my $sthOracle = $dbh->prepare("SELECT CMDB_APP_ID, OP_COM, Class FROM managed_app where CMDB_APP_ID is not NULL") || die $dbh->errstr;
...
#			$LenAppData = length($AppData);
			...
			if (exists($CMDB_Hash{$CMDB_ID}))
			{
				...
				{
#					if ($Verbose){print "CMDB_ID = $CMDB_ID   MMadoptdata = $AppData CMDB_data = $CMDB_AppData   $LenAppData\n";}
					logit ("OP_COM changed for Class $CLASS_ID from $AppData to $CMDB_AppData \n");
					if($Testing != 1)
					{
						my $sthOracle = $dbh->prepare("update managed_app set OP_COM = '$CMDB_AppData', CMDB_UPDATE = sysdate  where CMDB_APP_ID = $CMDB_ID") || die $dbh->errstr;
						$sthOracle->execute() || die $sthOracle->errstr;
					}
				}
			}
...
#	$dbh = DBI->connect( "dbi:Oracle:database=DCMDBV2;HOST=sldn1339dor.ldn.swissbank.com;PORT=1524;SID=DCMDBV2",
	...
	my $sthOracle = $dbh->prepare("SELECT Application_id, OP_COM FROM aradmin.ubs_esm_cmdb_appsys ") || die $dbh->errstr;

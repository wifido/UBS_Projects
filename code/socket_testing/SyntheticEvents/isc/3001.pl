...
#		$internet_addr = inet_aton("stmsktdev2.stm.swissbank.com");
	...
	if(length($internet_addr) > 0 )
	{
		...
		if($MessageConnected == 1)
		{
			...
			print TO_SERVER "'MessageText' 'Directory /vol/v10/dyn-RISK/data/RISK/Monitor/var/inc/cool/ reached the maxdirsize limit.'\n"; 
			...
			print TO_SERVER "'AlertGroup' 'maxdirsize'\n";

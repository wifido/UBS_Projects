...
#		$internet_addr = inet_aton("stmsktdev2.stm.swissbank.com");
	...
	if(length($internet_addr) > 0 )
	{
		...
		if($MessageConnected == 1)
		{
			...
			print TO_SERVER "'MessageText' 'The /w99 volume is 90.10% full (using 2.44 TB of 2.71 TB).There is 536 GB of spare disk space available for expanding the volume.'\n"; 
			...
			print TO_SERVER "'AlertGroup' 'volume'\n";
			print TO_SERVER "'OwnerGID' '542'\n";
			print TO_SERVER "'Domain' 'Domain'\n";

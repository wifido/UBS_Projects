...
#	print $socket "'Severity' 'FATAL'\n";
	...
	print $socket "'EventIdentifier' 'JaneschTesting'\n";
	print $socket "'SAPname' 'SAPname'\n";
	print $socket "'Subclass' 'WEBSPHERE'\n";
	print $socket "'ClassID' '2070993'\n";
	print $socket "'MessageText' 'Process state unknown - JaneschTesting dummy event please ignore'\n"; 
	print $socket "'EventType' '1'\n";
	print $socket "'EventTime' '$time'\n";
	print $socket "'Component' 'Process'\n";
	print $socket "'Stream' 'Stream'\n";
	print $socket "'Resolution' 'False'\n";
	print $socket "'Location' 'Londom'\n";

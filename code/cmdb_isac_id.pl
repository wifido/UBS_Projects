...
#	                        "reporter",
	...
	while (my (@nodeOracle) = $sthOracle->fetchrow_array) 
	{
			...
			if (exists($CMDB_Hash{$CMDB_ID}))
			{
				...
				$CMDB_AppData =~ s/'/ /;
#				print "CMDB_ID = $CMDB_ID   MMadoptdata = \'$AppData\' CMDB_data = $CMDB_AppData   $LenAppData $Testing\n";

				if($AppData ne $CMDB_AppData)
				{
					if ($Verbose){print "CMDB_ID = $CMDB_ID   MMadoptdata = \'$AppData\' CMDB_data = $CMDB_AppData   $LenAppData $Testing\n";}
					logit ("ISAC_APP_ID changed for Class $CLASS_ID from $AppData to $CMDB_AppData \n");
					if($Testing != 1)
					{
						my $sthOracle = $dbh->prepare("update managed_app set ISAC_APP_ID = '$CMDB_AppData', CMDB_UPDATE = sysdate where CMDB_APP_ID = $CMDB_ID") || die $dbh->errstr;
						$sthOracle->execute() || die $sthOracle->errstr;
					}
				}
			}
...
#	$dbh = DBI->connect( "dbi:Oracle:database=DCMDBV2;HOST=sldn1339dor.ldn.swissbank.com;PORT=1524;SID=DCMDBV2",
	...
	my $sthOracle = $dbh->prepare("SELECT Application_id, UBS_UNIQUEID FROM aradmin.ubs_esm_cmdb_appsys where UBS_UNIQUEID is not NULL") || die $dbh->errstr;
...
#			$CMDB_DATALEN = length($CMDB_DATA);
#			print "CMDB_ID = $CMDB_ID   CMDB_DATALEN = $CMDB_DATALEN\n";
#			if($CMDB_DATALEN)
#			{

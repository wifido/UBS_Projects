...
#	                        "reporter",
	...
	my $sthOracle = $dbh->prepare("SELECT CMDB_APP_ID, APPLICATION_LIFECYCLE_STATUS, Class FROM managed_app where CMDB_APP_ID is not NULL") || die $dbh->errstr;
	...
	while (my (@nodeOracle) = $sthOracle->fetchrow_array) 
	{
			...
			$LenAppLifStat = length($AppLifStat);
			$CLASS_ID = $nodeOracle[2];
			if (exists($CMDB_Hash{$CMDB_ID}))
			{
				$CMDB_APP_rtb_man = $CMDB_Hash{$CMDB_ID};
				$CMDB_APP_rtb_man =~ s/'/ /;
				if($AppLifStat ne $CMDB_APP_rtb_man)
				{
					if ($Verbose){print "CMDB_ID = $CMDB_ID   AppLifStat = \'$AppLifStat\' CMDB_APP_rtb_man = $CMDB_APP_rtb_man   $LenAppLifStat\n";}
					logit ("APPLICATION_LIFECYCLE_STATUS changed for Class $CLASS_ID from $AppLifStat to $CMDB_APP_rtb_man \n");
					if($Testing != 1)
					{
						my $sthOracle = $dbh->prepare("update managed_app set APPLICATION_LIFECYCLE_STATUS = '$CMDB_APP_rtb_man', CMDB_UPDATE = sysdate where CMDB_APP_ID = $CMDB_ID") || die $dbh->errstr;
						$sthOracle->execute() || die $sthOracle->errstr;
					}
				}
			}
...
#	$dbh = DBI->connect( "dbi:Oracle:database=DCMDBV2;HOST=sldn1339dor.ldn.swissbank.com;PORT=1524;SID=DCMDBV2",
	...
	my $sthOracle = $dbh->prepare("SELECT Application_id, STATUS FROM aradmin.ubs_esm_cmdb_appsys ") || die $dbh->errstr;
	...
	while (my (@nodeOracle) = $sthOracle->fetchrow_array) 
	{
		...
		{
			$CMDB_ID = $nodeOracle[0];
			$CMDB_STATUS = $nodeOracle[1];
			$CMDB_Hash{$CMDB_ID} = $CMDB_STATUS;
		}
	}
	$sthOracle->finish;
	$dbh->disconnect();

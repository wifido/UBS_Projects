...
#	                        "reporter",
	...
	my $sthOracle = $dbh->prepare("SELECT CMDB_APP_ID, APP_NAME, Class FROM managed_app where CMDB_APP_ID is not NULL") || die $dbh->errstr;
	...
	while (my (@nodeOracle) = $sthOracle->fetchrow_array) 
	{
			...
			if (exists($CMDB_Hash{$CMDB_ID}))
			{
				...
				if($AppData ne $CMDB_AppData)
				{
					...
					logit ("APP_NAME changed for Class $CLASS_ID from $AppData to $CMDB_AppData \n");
					if($Testing != 1)
					{
						my $sthOracle = $dbh->prepare("update managed_app set APP_NAME = '$CMDB_AppData', CMDB_UPDATE = sysdate where CMDB_APP_ID = $CMDB_ID") || die $dbh->errstr;
						$sthOracle->execute() || die $sthOracle->errstr;
					}
				}
			}
...
#	$dbh = DBI->connect( "dbi:Oracle:database=DCMDBV2;HOST=sldn1339dor.ldn.swissbank.com;PORT=1524;SID=DCMDBV2",
	...
	my $sthOracle = $dbh->prepare("SELECT Application_id, APP_NAME FROM aradmin.ubs_esm_cmdb_appsys ") || die $dbh->errstr;
